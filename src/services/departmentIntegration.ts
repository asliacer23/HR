...
const fallbackCacheKeys = new Set<string>();
...
async function fetchApiData<T>(
  ...
  const payload = (text ? JSON.parse(text) : { ok: response.ok }) as ApiResponse<T>;

  if (!response.ok || !payload.ok) {
    throw new Error(payload.message || `Request failed (${response.status})`);
  }

  fallbackCacheKeys.add(url);
...
export async function getFlowEventStatus(eventId?: string, correlationId?: string): Promise<FlowEventStatus> {
  try {
    if (resolveSupabaseUrl() && resolveSupabaseKey()) {
      return await callSupabaseRpc<FlowEventStatus>('get_department_flow_status', {
...
export async function getIntegrationRegistry(sourceDepartment?: string): Promise<IntegrationRegistry> {
  try {
    if (resolveSupabaseUrl() && resolveSupabaseKey()) {
      return await callSupabaseRpc<IntegrationRegistry>('get_department_integration_registry', {
        _source_department_key: sourceDepartment || null
      });
    }
